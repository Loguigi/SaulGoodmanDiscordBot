...
public class LevelCommands : ApplicationCommandModule 
{
    ...
    public async Task LevelLeaderboard(InteractionContext ctx) 
    {
        try 
        {
            var leaderboard = new Levels(ctx.Guild).GetInteractivity();
            ...
            foreach (var user in leaderboard.GetPage()) 
            {
            ...
            await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder(leaderboard.AddPageButtons().AddEmbed(embed)));
